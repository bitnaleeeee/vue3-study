          ...
          <div class="inner-style">
            <ul v-if="isText">
              ...
              <li
                ...
                v-bind:class="{
                  selected: selectedRowIndex === services.length && isText,
                }"
              >
        ...
        <div v-else class="new-user-table">
          <div class="user-info-read-wrap">
            ...
            <div class="customer-info-container">
              ...
              <div class="user-id-card">
                <label>
                  ...
                  ></i>
                  <input style="border: none" placeholder="성명" />
                </label>
                <label>
                  <i
                    class="bx bx-user"
            ...
            <div class="service_logs_container">
              ...
              <div class="service-logs-card" v-for="n in 2">
                <div class="logs-card-menu" style="color: #ff9100">
                  예약 상담
                </div>
                <div class="logs-card-text-wrap">
                  <div class="logs-card-text">문자 전송</div>
                  <div class="logs-card-time">
                    {{ new Date().toLocaleDateString() }}
                    {{ new Date().toLocaleTimeString() }}
                  </div>
...
const uniqueStatuses = computed(() => {
  return Array.from(
    new Set(tableData.value.map((row) => row[tableHeaders.length - 2]))
  );
});
...
const displayedTableData = computed(() => {
  ...
  if (selectedFilter.value) {
    filteredData = tableData.value.filter(
      (row) => row[tableHeaders.length - 2] === selectedFilter.value
    );
  } // 상담상태 선택 필터링
