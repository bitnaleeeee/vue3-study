<template>
    <div class="hck-autocomplete" :class="random" @keyup="keyup">
        <input v-model="InputValue"  style="width:100%" @keyup="openExpand('force')" @click="openExpand('force')"/>
        ...
        <div v-if="open" class="expand" ref="Expand" >
            <UIButton class="item" style="text-align:center; color:var(--primary)" @click="addItem">
                 추가하기
            </UIButton>
            <div class="item"
            :class="[item == Selected ? 'selected' : '', i == SelectedIndex ? 'highlight' : '']"
             v-for="(item,i) in Items" :key="i" @click="changeSelection(i)">
                {{item}}
            </div>
            <div>
            </div>
